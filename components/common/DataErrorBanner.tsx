...
const DataErrorBanner: React.FC<DataErrorBannerProps> = ({ error, onRetry }) => {
  ...
  const isSchemaError = error.includes("does not exist") || error.includes("schema cache");

  const title = t('app.fetchError');
  const message = isSchemaError ? t('app.schemaError') : error;
      ...
      <div className="container mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3 text-center sm:text-left">
          ...
          <div>
            ...
            <p className={`text-sm mt-1 ${isSchemaError ? '' : 'font-mono bg-brand-dark/30 p-1 rounded'}`}>{message}</p>
          </div>
        </div>
