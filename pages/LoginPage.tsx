...
import { loginUser, getCurrentUser, signInWithGoogleRedirect } from "../services/authService";
...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin, onNavigate }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      await loginUser(email, password); // Appwrite login
      const user = await getCurrentUser(); // Appwrite current user
      alert(`Bienvenue ${user?.name || user?.email || "utilisateur"}`);
  ...
  const handleGoogleSignInClick = async () => {
    setError("");
    try {
      await signInWithGoogleRedirect(); // Appwrite OAuth2 redirect
    } catch (err) {
